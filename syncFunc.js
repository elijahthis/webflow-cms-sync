		...
		while (startIndex < updatedAirtableProfiles.length) {
			...
			const webflowUpdatePromises = batchAirtableProfiles.map(
				async (airtableProfile) => {
					...
					if (webflowProfile) {
						// response = await updateWebflowCMSItem(webflowProfile?.id, {
						// 	fieldData: {
...
const directorySyncFunc = async (lastCheckedDate) => {
	try {
		const updatedAirtableProfiles =
			await fetchRecentlyUpdatedDirectoriesFromAirtable(lastCheckedDate);
